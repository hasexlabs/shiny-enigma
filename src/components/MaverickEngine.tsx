...
export default function MaverickEngine() {
  ...
  const initializeDefaultThread = () => {
    // Check for onboarding results to generate personalized first message
    let firstMessage = "System initialized. Welcome to HASEX CORE // MAVERICK Command Intel.\n\nI am MAVERICK AI, your execution-oriented guide vector. Ready to analyze messy logs, scan schemas, construct layouts, and compile models using selective cascading parameters. Input your instructions:";
    
    try {
      const onboardingData = localStorage.getItem("hasex_evaluation");
      if (onboardingData) {
        const parsed = JSON.parse(onboardingData);
        
        // Generate personalized message based on user's path
        if (parsed.wantToBecome && parsed.wantToBecome !== "Not Sure Yet") {
          firstMessage = `I see you want to become ${parsed.wantToBecome}.\n\nMost people say they want to become something.\n\nVery few can explain what they are doing today to become it.\n\nLet's dive deeper.\n\nWhy this path?\n\nWhat have you already done?\n\nWhat is currently stopping you?\n\nBe specific.`;
        } else if (parsed.pathType === "not_sure_yet") {
          firstMessage = `I see you're still exploring.\n\nGood.\n\nChoosing the wrong path is worse than taking extra time to choose the right one.\n\nLet's figure this out.\n\nWhat topics do you naturally spend time learning about?\n\nWhat problems do you enjoy solving?\n\nWhat kind of future sounds exciting to you?`;
        }
        
        // Clear onboarding data after generating message to avoid showing it again
