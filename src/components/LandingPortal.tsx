...
export default function LandingPortal({ onEnter }: LandingPortalProps) {
  ...
  // Authentic Supabase User State bindings
  const { user: currentUser, loading: isAuthChecking, signInWithGoogle } = useAuth();
            ...
            <div className="flex flex-col items-center gap-3 w-full">
              ...
              {!currentUser && (
                <button
                  ...
                  className="text-[10px] text-[#00f0ff] hover:text-white uppercase tracking-widest font-bold transition-colors cursor-pointer select-none bg-transparent border-none mb-1"
                >
                  {isAuthWorking ? 'CONNECTING...' : 'Sign in'}
                </button>
              )}

              {/* Conditional Guest/Enter button */}
              <button
                type="button"
                onClick={triggerEntry}
                disabled={isAuthWorking}
                className={`
                  group relative flex items-center justify-center w-full py-4 transition-all duration-300 active:scale-95 cursor-pointer
                  ${currentUser ? 'bg-black/40' : 'bg-black/20 opacity-50 hover:opacity-60'}
                `}
              >
                {/* Button Border/Glow */}
                <div className={`absolute inset-0 border transition-colors duration-500 ${currentUser ? 'border-[#00f0ff]/25 group-hover:border-[#00f0ff]/65' : 'border-[#00f0ff]/10'}`}></div>
                {currentUser && <div className="absolute inset-[-4px] border border-[#00f0ff]/5 opacity-0 group-hover:opacity-100 transition-opacity duration-700 scale-105"></div>}
                <div className="text-[11px] text-[#dbfcff] font-bold tracking-[0.3em] uppercase z-10 flex items-center gap-2">
                  {currentUser ? (
                    <div className="flex flex-col items-center">
                      <span className="text-[9px] text-[#00f0ff]/80 mb-1">{currentUser.email}</span>
                ...
                {currentUser && <div className="absolute top-0 left-0 w-full h-[1px] bg-[#00f0ff]/40 -translate-y-2 opacity-0 group-hover:opacity-100 group-hover:translate-y-full transition-all duration-[2s] ease-in-out"></div>}
              </button>
            </div>
          )}
        </div>
